...
`earthquake-monitoring` API'si için React + Vite + Leaflet ön yüzü. Tek backend ön
...
| `scientist` | `science123` | SCIENTIST  | Analiz, içe aktarma, kayıt  |
| `viewer`    | `viewer123`  | PUBLIC     | Sadece okuma                |

Giriş ekranında hesap çiplerine dokununca alanlar dolar. Rol bazlı yetkiler
backend'in `SecurityConfig` kuralarıyla birebir aynı: UI yalnızca rolün izin verdiği
eylemleri gösterir, sunucu da ikinci savunma hattı olarak uygular.

## Görünümler

- **İzleme** — risk renkli olay haritası (Leaflet), olay listesi ve metrik şeridi.
  Bilim insanı/yönetici akışları içe aktarır; yönetici olay siler.
- **Algılama** — sentetik bir sismik sinyal penceresi üretip `/api/detection/analyze`'a
  yollar. İmza öğesi: gerçek sismograf izi çizimi. İstasyon Yalova ön-dolu (YLV-01).
...
src/
  ...
  lib/        risk · format · signal (sentetik sinyal üreteci)
  components/ Toast · ui · Seismograph · SeismicMap · DataContext
  views/      LoginView · Shell · Monitor · Detection · Disaster · Report
